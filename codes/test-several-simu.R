...
source("dataGenerator-v7.R")
source("Sim2MSnSet-corrected.R")
source("groupttest.R")
...
propSharedList <- c(0, 0.01, 0.05, 0.1, 0.2, 0.33, 0.5, 0.67);

for(propShared in propSharedList){
  print(c("Shared peptides proportion: ", propShared))
  for(nn in 1:n.simu){
    print(c("iteration: ", nn))
    source('test-one-simu.R')
  ...
  for(ii in 1:length(list.scores)){
    ...
    if(ii == 1){
      pdf(file=sprintf('pr-%.2fpropShared-simu.pdf', propShared))
      par(mar=c(5,5,4,2))
  ...
  pdf(paste('calibration - s=',s1,', simu - Propshared=', propShared,'.pdf', sep=""))
  
  
  plot(log10(sam.pv), log10(sam.fpr), main=paste('Calibration plot, s=',s1,', simu - Propshared=', propShared, sep=""), xlab='log10 p-value threshold', ylab='Actual test level (log10)', 
