...
for(Nshare in NshareList){          
  ...
  for(ii in 1:length(list.scores)){
    ...
    if(ii == 1){
      pdf(file=sprintf('pr-%.0fNshare-UPSx25.pdf', Nshare))
      par(mar=c(5,5,4,2))
  ...
  pdf(paste('calibration - s=',s1,', upsx25 - Nshare=', Nshare,'.pdf', sep=""))
  
  
  plot(log10(sam.pv), log10(sam.fpr), main=paste('Calibration plot, s=',s1,', Exp1_R25_pept - Nshare=', Nshare, sep=""), xlab='log10 p-value threshold', ylab='Actual test level (log10)', 
